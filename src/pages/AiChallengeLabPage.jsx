...
export default function AiChallengeLabPage({ onShowToast }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (userProfile && !userProfile.badges?.includes('AIExplorer')) {
        const res = await updateProgression(0, 'AIExplorer');
        if (res) {
    ...
    try {
      ...
      if (usePhishing) {
        ...
        adapted = {
          ...
          choices: [
            { id: "A", label: "Treat as Safe" },
            { id: "B", label: "Treat as Phishing" }
          ],
          correctChoice: selected.isPhishing ? "B" : "A",
        ...
        adapted = {
          ...
          choices: [
            { id: "A", label: selected.choices[0].label },
            { id: "B", label: selected.choices[1].label }
          ],
          correctChoice: selected.choices[0].isCorrect ? "A" : "B",
          explanation: selected.explanation
        };
      }
  ...
  const handleSubmit = async () => {
    if (!chosenChoice || !challenge) return;

    const userCorrect = chosenChoice === challenge.correctChoice;
    ...
    try {
      ...
      const res = await updateProgression(xpEarned);
      if (res) {
        handleProgressionToasts(res);
      }
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
      ...
      {challenge && !loading && !error && (
        <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-[#1F242F] bg-[#171A21] border border-[#1F242F] rounded-lg overflow-hidden min-h-[550px]">
          ...
          {/* RIGHT COLUMN: MAIN BRIEFING CONSOLE */}
          <div className="col-span-2 flex flex-col bg-[#0F1115] p-6">
            <div className="flex flex-col gap-5 flex-grow">
              
              {/* Console header envelope */}
              <div className="border border-[#1F242F] bg-[#171A21] rounded-t-lg p-4 flex flex-col gap-2 font-sans">
                <div className="text-xs flex items-center gap-2 text-[#9CA3AF]">
                  <span className="font-semibold text-[#F3F4F6] w-14">Incident:</span>
                  <span className="text-[#F3F4F6] font-bold">{challenge.title}</span>
                </div>
                <div className="text-xs flex items-center gap-2 text-[#9CA3AF] border-t border-[#1F242F]/60 pt-2 mt-1">
                  <span className="font-semibold text-[#F3F4F6] w-14">Status:</span>
                  {submitted ? (
                    isCorrect ? (
                      <span className="text-[10px] font-bold text-[#22C55E]">RESOLVED (SECURE)</span>
                    ) : (
                      <span className="text-[10px] font-bold text-[#EF4444]">RESOLVED (BREACHED)</span>
                    )
                  ) : (
                    <span className="text-[10px] font-bold text-[#3B82F6] animate-pulse">ACTIVE ANALYSIS...</span>
                  )}
                </div>
              </div>
              ...
              <div className="border border-[#1F242F] bg-[#171A21] rounded-b-lg p-6 font-mono text-xs leading-relaxed text-[#F3F4F6] break-words border-t border-[#1F242F]/50 flex flex-col gap-3">
                <span className="text-[#3B82F6] font-bold text-[10px] tracking-wider uppercase">[INCIDENT SCENARIO FEED]</span>
                ...
                {!submitted ? (
                  <div className="bg-[#171A21] border border-[#1F242F] p-5 rounded-lg flex flex-col gap-4">
                    ...
                    <p className="text-xs text-[#9CA3AF] -mt-1 leading-snug">
                      Evaluate the scenario above, then select your defensive action:
                    </p>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
                      {challenge.choices.map((choice) => {
                        const isSelected = chosenChoice === choice.id;

                        return (
                          <button
                            key={choice.id}
                            type="button"
                            onClick={() => setChosenChoice(choice.id)}
