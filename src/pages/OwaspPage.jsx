...
export default function OwaspPage({ onShowToast }) {
  ...
  const [answers, setAnswers] = useState(() => {
    try {
      const key = getStorageKey('cyberquest_owasp_answers', user?.uid);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const initialBadge = isFirstChallenge ? 'FirstInvestigation' : null;

      // Submit scenario score
      const res = await updateProgression(xpEarned, initialBadge, { module: 'OWASP Top 10 Defenses', detail: selectedScenario.topic });
      if (res) {
      ...
      if (currentAnsweredCount === scenarios.length) {
        ...
        // Completion Badges:
        // - OWASPExplorer: Earned for completing the module
        // - ThreatHunter: Earned for scoring 8/10+
        // - ApplicationGuardian: Earned for scoring 10/10
        let badgesToUnlock = ['OWASPExplorer'];
        let completionXP = 20; // base completion XP

        if (correctCount === 10) {
          completionXP += 50;
          badgesToUnlock.push('ApplicationGuardian');
          badgesToUnlock.push('ThreatHunter');
        ...
        setTimeout(async () => {
          try {
            const compRes = await updateProgression(completionXP, badgesToUnlock, {
              module: 'OWASP Top 10 Defenses',
            ...
            console.error("OWASP completion sync failed:", err);
          }
        }, 2200);
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
      ...
      <section className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#1F242F] pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-[#F3F4F6] tracking-tight flex items-center gap-2">
            <Key className="w-8 h-8 text-[#3B82F6]" /> OWASP Top 10 Defenses
          </h1>
          <p className="text-[#9CA3AF] text-sm mt-1">
            Review code-level vulnerabilities and architectural choices to design and deploy secure applications.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-[#1F242F] bg-[#171A21] border border-[#1F242F] rounded-lg overflow-hidden min-h-[620px]">
        ...
        {/* LEFT COLUMN: SCENARIOS SIDEBAR CHECKLIST */}
        <div className={`col-span-1 flex flex-col ${mobileTab === 'reader' ? 'hidden lg:flex' : 'flex'}`}>
          <div className="p-4 border-b border-[#1F242F] bg-[#171A21] flex justify-between items-center">
            <span className="text-sm font-bold text-[#F3F4F6] tracking-tight flex items-center gap-2 font-sans">
              <ShieldCheck className="w-4 h-4 text-[#3B82F6]" />
              AppSec Lab Scenarios ({answeredCount}/{totalScenarios})
            </span>
            {isAllCompleted && (
              <span className="text-[10px] font-bold bg-[#22C55E]/10 text-[#22C55E] border border-[#22C55E]/20 px-2 py-0.5 rounded font-mono">
                COMPLETED
              </span>
            )}
          </div>
        ...
        {/* RIGHT COLUMN: LAB SCENARIO CONSOLE */}
        <div className={`col-span-2 flex flex-col bg-[#0F1115] ${mobileTab === 'list' ? 'hidden lg:flex' : 'flex'}`}>
          {selectedScenario ? (
            <>
              ...
              <div className="p-4 border-b border-[#1F242F] bg-[#171A21] flex justify-between items-center gap-4">
                ...
                <div className="hidden lg:flex items-center gap-2">
                  <span className="text-xs text-[#9CA3AF]">Vulnerability & Defense Validation</span>
                </div>
              ...
              {isAllCompleted && (
                <div className="mx-6 mt-6 bg-[#171A21] border border-[#3B82F6]/20 p-5 rounded-lg flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-sm text-[#F3F4F6]">AppSec Certification Issued</h4>
                      <p className="text-xs text-[#9CA3AF] mt-0.5">
                        You finished the OWASP Top 10 path. Score: <strong className="text-[#F3F4F6] font-semibold">{correctCount} / {totalScenarios}</strong> correct.
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div className="p-6 flex-grow overflow-y-auto max-h-[500px]">
                ...
                {/* Simulated Incident briefing headers */}
                <div className="border border-[#1F242F] bg-[#171A21] rounded-t-lg p-4 flex flex-col gap-2 font-sans border-b-0">
                  <div className="text-xs flex flex-wrap items-center gap-2 text-[#9CA3AF]">
                    <span className="font-semibold text-[#F3F4F6] w-16">Threat Area:</span>
                  ...
                  <div className="text-xs flex items-center gap-2 text-[#9CA3AF]">
                    <span className="font-semibold text-[#F3F4F6] w-16">Objective:</span>
                ...
                <div className="border border-[#1F242F] bg-[#171A21] rounded-b-lg p-6 font-mono text-xs leading-relaxed text-[#F3F4F6] break-words border-t border-[#1F242F]/50 flex flex-col gap-3">
                  <span className="text-[#3B82F6] font-bold text-[10px] tracking-wider uppercase flex items-center gap-1.5">
                    <Terminal className="w-3.5 h-3.5" /> [SECURITY THREAT BRIEFING]
                  </span>
                  <p>{selectedScenario.scenarioText}</p>
                </div>

                {/* Interactive Decisions Panel (Code-focused options) */}
                <div className="mt-6">
                  {!currentAnswer ? (
                    <div className="bg-[#171A21] border border-[#1F242F] p-5 rounded-lg flex flex-col gap-4">
                      <h4 className="text-xs font-semibold text-[#F3F4F6] uppercase tracking-wider flex items-center gap-1.5 font-sans">
                        <Info className="w-4 h-4 text-[#3B82F6]" /> Implementation Decision
                      </h4>
                      <p className="text-xs text-[#9CA3AF] -mt-1 leading-snug font-sans">
                        Select the correct code block or architecture approach that secures this workflow:
                      </p>
                        ...
                        {selectedScenario.choices.map((choice) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-start text-left p-4 rounded-md border transition-all cursor-pointer focus:outline-none gap-3 min-h-[60px] ${
                                isSelected
                                  ? 'bg-[#3B82F6]/5 border-[#3B82F6] text-[#F3F4F6] shadow-sm'
                            ...
                            >
                              <span className={`w-4 h-4 rounded-full border flex-shrink-0 mt-0.5 flex items-center justify-center ${
                                isSelected ? 'border-[#3B82F6] bg-[#3B82F6]' : 'border-[#4B5563]'
                              }`}>
                                {isSelected && <span className="w-1.5 h-1.5 bg-[#0F1115] rounded-full"></span>}
                              </span>
                              <span className="leading-relaxed break-words">{choice.label}</span>
                            </button>
                          );
                        })}
                      </div>
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className={`p-4 border rounded-lg flex items-start gap-3 ${
                        ...
                        <div className="text-xs">
                          <h4 className="font-bold text-sm mb-1 font-sans">
                            {currentAnswer.isCorrect ? 'Safe Implementation Verified' : 'Vulnerable Implementation Detected'}
                          </h4>
                          <p className="text-[#9CA3AF] leading-relaxed">
                            {currentAnswer.isCorrect 
                              ? 'Your selected decision successfully mitigates the OWASP threat vector.' 
                              : 'This selection leaves the system exposed to attack.'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-[#171A21] border border-[#1F242F] p-5 rounded-lg flex flex-col gap-3">
                        <h4 className="text-xs font-bold text-[#F3F4F6] uppercase tracking-wider font-sans">
            ...
            <div className="flex-grow flex flex-col items-center justify-center text-center p-8 gap-3">
              ...
              <p className="text-sm text-[#9CA3AF]">Select an OWASP threat scenario from the checklist to start your AppSec lab.</p>
            </div>
          )}
        </div>
